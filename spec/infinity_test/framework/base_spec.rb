    ...
    describe Base do
      ...
      describe '#run_test' do
        ...
        context 'find a file' do
          it 'call run strategy to continuous test server' do
            expect(base.hike).to receive(:find)
              .with('framework/base_test.rb')
              .and_return('framework/base_test.rb')
            expect(continuous_test_server).to receive(:rerun_strategy)
              .with('framework/base_test.rb')
          ...
          it 'use the extension name from core base' do
            ...
            expect(base.hike).to receive(:find)
              .with('framework/base_test.py')
              .and_return('framework/base_test.py')
            expect(continuous_test_server).to receive(:rerun_strategy)
              .with('framework/base_test.py')
            base.run_test(double(path: 'framework/base'))
          end
        ...
        context 'when do not find any files' do
          it 'do not call rerun strategy' do
            expect(base.hike).to receive(:find)
              .with('test_framework/rspec_test.rb')
              .and_return(nil)
