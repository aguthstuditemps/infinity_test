  ...
  module Core
    class Options
      ...
      def ruby_strategy(option)
        option.on(
          '--ruby strategy',
          'Ruby Manager. Ex.: auto_discover, rvm, rbenv, ruby_default'
        ) do |strategy|
      ...
      def options_to_added_in_the_command(option)
        option.on(
          '--options=options',
          'Pass the options to be added. Ex. -Ilib-Itest-Iapp'
        ) do |options|
      ...
      def test_framework_to_be_run(option)
        option.on(
          '--test library',
          'Ex.: rspec, test_unit, bacon (auto_discover is the default)'
        ) do |library|
      ...
      def app_framework(option)
        option.on(
          '--framework library',
          'Framework to be run. Ex.: auto_discover, rails, rubygems, padrino'
        ) do |library|
      ...
      def verbose_mode(option)
        option.on(
          '--no-verbose',
          'Does not print commands before executing them'
        ) do
          @verbose = false
        end
      end
      ...
      def skip_bundler(option)
        option.on('--no-bundler', 'Bypass Infinity Test Bundler support') do
          @bundler = false
        end
      end
      ...
      def help(option)
        option.on_tail('--help', 'You are looking at it.') do
          print option.help
          exit
        end
      end
